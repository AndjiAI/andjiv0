...
export type AndjiClientOptions = {
  // Provide an API key or set the andji_API_KEY environment variable.
  apiKey?: string
  cwd?: string
...
export class AndjiClient {
  public cwd: string

  private readonly websocketHandler: WebSocketHandler
  private readonly overrideTools: NonNullable<
    AndjiClientOptions['overrideTools']
  >
  private readonly fingerprintId = `andji-sdk-${Math.random().toString(36).substring(2, 15)}`
  ...
  constructor({ apiKey, cwd, onError, overrideTools }: AndjiClientOptions) {
    const foundApiKey = apiKey ?? process.env[API_KEY_ENV_VAR]
    if (!foundApiKey) {
      throw new Error(
        `andji API key not found. Please provide an apiKey in the constructor of AndjiClient or set the ${API_KEY_ENV_VAR} environment variable.`,
      )
    }
  ...
  /**
   * Run a andji agent with the specified options.
   *
   ...
   * @param projectFiles - (Optional) All the files in your project as a plain JavaScript object. Keys should be the full path from your current directory to each file, and values should be the string contents of the file. Example: { "src/index.ts": "console.log('hi')" }. This helps andji pick good source files for context.
