...
import { user } from '@andji/common/db/schema'
...
export function startDiscordBot() {
  ...
  client.on(Events.MessageCreate, async (message) => {
    ...
    if (message.system && message.type === 7) {
      try {
        await message.reply({
          content: `Hey there! Enter \`/link\` to connect your Discord account with andji (don't worry, only you can see it).`,
        })
      } catch (error) {
  ...
  client.on(Events.InteractionCreate, async (interaction: Interaction) => {
    ...
    if (command.commandName === 'link') {
      ...
      try {
        ...
        await command.reply({
          content:
            "Thanks! I've linked your Discord account to your andji account. You're all set! 🎉",
          ephemeral: true,
        })
      } catch (error) {
