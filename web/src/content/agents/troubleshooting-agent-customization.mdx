...
1. **Restart andji** to reload templates
...
```bash
# Restart andji to reload templates
andji
```
...
```json
{
  "id": "andjiAI/reviewer", // ✅ Exact match required
...
Restart andji to see validation errors:

```bash
andji  # Look for error messages on startup
```
...
1. **Check the logs**: Look for specific error messages when starting andji
2. **Simplify**: Remove customizations until it works, then add back gradually
3. **Community**: Join our [Discord](https://andji.com/discord) for real-time help
