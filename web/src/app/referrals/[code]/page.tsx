...
export const generateMetadata = async ({
  ...
  const title = referrerName
    ? `${referrerName} invited you to andji!`
    : 'Join andji with a referral bonus!'

  return {
    title,
    description:
      'Get bonus credits when you sign up for andji with this referral link.',
  }
}
...
export default async function ReferralPage({
  ...
  try {
    const baseUrl = env.NEXT_PUBLIC_andji_APP_URL || 'http://localhost:3000'
  ...
  return (
    <ReferralClientWrapper referralCode={code}>
      <CardWithBeams
        title={`${referrerDisplayName} invited you to andji!`}
