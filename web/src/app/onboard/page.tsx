...
import { MAX_DATE } from '@andji/common/old-constants'
...
const Onboard = async ({ searchParams = {} }: PageProps) => {
  ...
  if (!user) {
    ...
    return redirect(env.NEXT_PUBLIC_andji_APP_URL)
  }
  ...
  if (!authCode) {
    ...
    const successCard = CardWithBeams({
      title: 'Welcome to andji!',
      description: hasReferralInUrl
        ? "Once you've installed andji, you can close this window."
  ...
  if (receivedfingerprintHash !== fingerprintHash) {
    return CardWithBeams({
      ...
      content: (
        <p>
          Please try again and reach out to support@andji.com if the problem
          persists.
        </p>
      ),
    })
  ...
  if (expiresAt < Date.now().toString()) {
    return CardWithBeams({
      ...
      content: (
        <p>
          Please generate a new code and reach out to support@andji.com if
  ...
  if (didInsert) {
    ...
    const successCard = CardWithBeams({
      ...
      description: isReferralUser
        ? 'Follow the steps above to install andji, then you can close this window.'
