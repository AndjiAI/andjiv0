import { grantOrganizationCredits } from '@andji/billing'
import { CREDIT_PRICING } from '@andji/common/old-constants'
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const successUrl = `${env.NEXT_PUBLIC_andji_APP_URL}/orgs/${organization.slug}?purchase_success=true`
    const cancelUrl = `${env.NEXT_PUBLIC_andji_APP_URL}/orgs/${organization.slug}?purchase_canceled=true`
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `${credits.toLocaleString()} andji Credits`,
