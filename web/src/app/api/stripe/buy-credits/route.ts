import { getUserCostPerCredit, processAndGrantCredit } from '@andji/billing'
import db from '@andji/common/db'
import * as schema from '@andji/common/db/schema'
import { convertCreditsToUsdCents } from '@andji/common/util/currency'
import { generateCompactId } from '@andji/common/util/string'
import { stripeServer } from '@andji/common/util/stripe'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const checkoutSession = await stripeServer.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            currency: 'usd',
            product_data: {
              name: `andji Credits - ${credits.toLocaleString()}`,
      ...
      mode: 'payment',
      success_url: `${env.NEXT_PUBLIC_andji_APP_URL}/payment-success?session_id={CHECKOUT_SESSION_ID}&purchase=credits&amt=${credits}`,
      cancel_url: `${env.NEXT_PUBLIC_andji_APP_URL}/usage?purchase_canceled=true`,
      metadata: {
        userId: userId,
        credits: credits.toString(),
