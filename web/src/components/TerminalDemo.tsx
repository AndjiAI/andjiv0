import { sleep } from '@andji/common/util/promise'
...
const getIframeContent = (
  ...
  const fixedContent = `
    ...
    <p>Like the demo? Pls install andji so we can justify keeping this demo pls:</p>
    <code><pre>npm install -g andji</pre></code>
    `
...
const TerminalDemo = () => {
  ...
  const [terminalLines, setTerminalLines] = useState<React.ReactNode[]>([
    <TerminalOutput key="welcome">
      <span className="text-green-400 font-bold">andji CLI v1.5.0</span>
  ...
  const [previewContent, setPreviewContent] =
    useState<string>(`<div style="padding: 16px; border-radius: 8px;">
      <h1 class="text-xl font-bold">👋 Welcome to the andji Demo!</h1>
      ...
      <div style="margin-top: 16px; padding: 8px; background: rgba(74,222,128,0.1); border-radius: 4px; border-left: 3px solid rgba(74,222,128,0.5);">
        <p style="font-style: italic; font-size: 0.9em;">This is just a demo. Install andji to experience the full capabilities with your own projects!</p>
      </div>`)
    ...
    onMutate: (input) => {
      ...
      const newLines = [
        ...
        <TerminalOutput key={`files-${Date.now()}`}>
          <b className="text-green-400">andji:</b> Reading additional
          files...
          ...
          <TerminalOutput key={`help-${Date.now()}`}>
            <div className="bg-zinc-800/50 p-3 rounded-md border border-zinc-700/50 my-2">
              ...
              <p className="mt-3 text-green-400">
                ℹ️ This is a demo with limited functionality. Install andji
          ...
          <TerminalOutput key={`generic-response-${Date.now()}`}>
            <div className="my-2">
              ...
              <p className="text-white/60 text-sm italic mt-4">
                Install andji to get the full experience with your own
                projects!
              </p>
