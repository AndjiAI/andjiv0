...
import { disableLiveUserInputCheck } from '@andji/backend/live-user-inputs'
import { promptAiSdkStructured } from '@andji/backend/llm-apis/vercel-ai-sdk/ai-sdk'
import { models } from '@andji/common/old-constants'
import { withTimeout } from '@andji/common/util/promise'
import { generateCompactId } from '@andji/common/util/string'
...
import { andjiRunner } from './runners/andji'
...
export async function runSingleEval(
  ...
  codingAgent: 'andji' | 'claude',
  agent?: string,
): Promise<EvalRunJudged> {
  const startTime = new Date()
  const trace: andjiTrace[] = []
  ...
  try {
    ...
    // Initialize state
    let runner: Runner
    if (codingAgent === 'andji') {
      runner = new andjiRunner(
        {
    ...
    while (currentDecision === 'continue' && attempts < MAX_ATTEMPTS) {
      ...
      const renderedTrace = trace
        .map(
          ({ prompt, steps }) =>
            `You: ${prompt}\n\nandji:${steps.map(renderAgentStep).join('\n\n')}`,
        )
        .join('\n\n')
      ...
      try {
        agentResponse = await promptAiSdkStructured({
          messages: [
            {
              role: 'user',
              content: `You are an expert software engineer tasked with implementing a specification using andji, an AI coding assistant. Your goal is to prompt andji to implement the spec correctly. You are in a conversation with this coding agent.

Current spec to implement:
<spec>${evalCommit.spec}</spec>

Your conversation with andji so far:
...
1. 'continue' - Generate a follow-up prompt for andji
...
If deciding to continue, include a clear, focused prompt for andji in next_prompt. Note that andji does not have access to the spec, so you must describe the changes you want andji to make in a way that is clear and concise.
Explain your reasoning in detail.`,
      ...
      if (agentResponse.decision === 'continue') {
        ...
        // Track credits used
        totalCostUsd += andjiResult.totalCostUsd
        trace.push({ prompt, steps: andjiResult.steps })
      }
  ...
  const fileStates = getandjiFileStates(evalCommit.sha, projectPath)
...
if (require.main === module) {
  ...
  const codingAgent = args[2] || 'andji'
  if (!['andji', 'claude'].includes(codingAgent)) {
    throw new Error(`Invalid coding agent: ${codingAgent}`)
  }

  runGitEvals(evalDataPath, outputDir, codingAgent as 'andji' | 'claude')
    .then(() => {
      console.log('Done!')
      process.exit(0)
    })
