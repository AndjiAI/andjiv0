...
import type { GitEvalResultRequest } from '@andji/common/db/schema'

const DEFAULT_OUTPUT_DIR = 'git-evals'
const MOCK_PATH = 'git-evals/eval-result-andji-mock.json'
const API_BASE = 'https://www.andji.com/'

class RunEvalSetCommand extends Command {
  static description = 'Run evaluation sets for andji'
  ...
  static flags = {
    ...
    agent: Flags.string({
      description: 'andji agent id to use',
      default: 'base',
    }),
...
async function runEvalSet(options: {
  ...
  const evalConfigs: EvalConfig[] = [
    {
      name: 'andji',
      evalDataPath: path.join(__dirname, 'eval-andji2.json'),
      outputDir,
    },
    {
      name: 'manifold',
