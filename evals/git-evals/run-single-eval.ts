...
import {
  setProjectRoot,
  setWorkingDirectory,
} from '@andji/npm-app/project-files'
import { recreateShell } from '@andji/npm-app/terminal/run-command'
...
class RunSingleEvalCommand extends Command {
  ...
  static examples = [
    '$ bun run-single-eval --eval-file eval-andji.json --commit-index 0',
    '$ bun run-single-eval --eval-file eval-manifold.json --commit-sha abc123',
    '$ bun run-single-eval --eval-file eval-andji.json --commit-index 5 --output results.json',
    '$ bun run-single-eval --eval-file eval-andji.json --commit-index 0 --agent base2',
  ]

  static flags = {
    'eval-file': Flags.string({
      char: 'f',
      description: 'Path to the eval JSON file (e.g., eval-andji.json)',
      required: true,
    }),
...
async function runSingleEvalTask(options: {
  ...
  if (!['andji', 'claude'].includes(codingAgentStr)) {
    throw new Error(`Invalid coding agent: ${codingAgentStr}`)
  }
  const codingAgent = codingAgentStr as 'andji' | 'claude'
