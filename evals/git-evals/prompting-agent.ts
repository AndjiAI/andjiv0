...
import type { AgentDefinition } from '@andji/common/templates/initial-agents-dir/types/agent-definition'
import type { z } from 'zod/v4'
import { AndjiClient } from '../../sdk/src'
  ...
  outputSchema: {
    ...
    properties: {
      ...
      next_prompt: {
        type: 'string',
        description:
          'The next prompt to send to andji (required if decision is continue)',
      },
    },
    required: ['decision', 'reasoning'],
  },
  systemPrompt: `You are an expert software engineer tasked with implementing a specification using andji, an AI coding assistant. Your goal is to prompt andji to implement the spec correctly.
...
1. 'continue' - Generate a follow-up prompt for andji the coding agent
...
/**
 * Get the next evaluation prompt using the andji SDK
 */
export async function getNextEvalPrompt({
  ...
  const prompt = `You are in a conversation with andji, an AI coding assistant.
Your conversation with andji so far:
<conversation>${conversationHistory}</conversation>
    
You need to implement the spec via prompting andji the coding agent. You only have ${attemptsRemaining} turns left to implement the spec.
