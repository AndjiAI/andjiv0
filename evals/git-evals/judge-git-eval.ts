...
function buildAnalysisPrompt(
  ...
  // Build andji changes section
  const andjiChanges = evalRun.gitDiff
  ...
  const traceContent =
    ...
    evalRun.trace
      .map(({ prompt, steps }) =>
        `Prompt: ${prompt}\n\nandji Steps: ${JSON.stringify(steps)}`.trim(),
      )
      .join('\n\n')

  return `You are an expert software engineer tasked with analyzing and scoring the code quality of changes made by an AI coding assistant (andji). Please analyze and compare both the attempted changes and the ground truth changes.
