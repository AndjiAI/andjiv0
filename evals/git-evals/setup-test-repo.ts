...
export async function setupTestRepo(
  ...
  try {
    ...
    const githubToken = process.env.andji_GITHUB_TOKEN
    ...
    if (
      ...
      console.error(
        '1. Verify andji_GITHUB_TOKEN environment variable is set',
      )
      console.error(
      ...
      const token = process.env.andji_GITHUB_TOKEN
      if (token) {
        console.error(
          `Token format: ${token.substring(0, 10)}... (length: ${token.length})`,
        )
      } else {
        console.error('andji_GITHUB_TOKEN environment variable is not set')
      }
    }
