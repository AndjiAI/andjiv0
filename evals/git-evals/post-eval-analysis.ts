...
import { countTokens } from '@andji/backend/util/token-counter'
...
Files Changed by andji: ${run.gitDiff || 'None'}
...
`
  ...
  return `You are an expert software engineering manager analyzing the performance of andji, an AI coding assistant. You have been given the results of a comprehensive evaluation where andji attempted to implement various coding tasks.

Your goal is to identify the most important problems that need to be solved to improve andji's performance, prioritized by impact and frequency. Focus on actionable development priorities that the engineering team can work on.
...
/**
 * Analyzes eval results to identify priority problems for andji development
