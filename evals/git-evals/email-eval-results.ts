import { sendBasicEmail } from '@andji/internal/loops'
...
function formatEvalSummaryForEmail(
  ...
  const subject = `andji Eval Results - ${title ? title : new Date().toLocaleDateString()} - Score: ${avgOverallScore.toFixed(1)}/10 | Cost: ${avgCostUsd.toFixed(3)} | ${avgRuntimeSec.toFixed(1)}s`
