...
import { isDir } from '@andji/common/util/file'
...
export async function validateAgent(
  ...
  try {
    ...
    // Use helper to create headers with x-andji-api-key
  ...
  public async printInitialPrompt({
    ...
    if (this.printMode) {
      if (!client.user && !process.env[API_KEY_ENV_VAR]) {
        printModeLog({
          type: 'error',
          message: `Print mode requires authentication. Please run "andji login" or set the ${API_KEY_ENV_VAR} environment variable first.`,
        })
        process.exit(1)
      }
    } else {
