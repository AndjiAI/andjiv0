...
import {
  ...
  UsageReponseSchema,
} from '@andji/common/actions'
import { READABLE_NAME } from '@andji/common/api-keys/constants'
import { AnalyticsEvent } from '@andji/common/constants/analytics-events'
import { andjiConfigFile as CONFIG_FILE_NAME } from '@andji/common/json-config/constants'
import {
  ASKED_CONFIG,
  ...
  API_KEY_ENV_VAR,
} from '@andji/common/old-constants'
import { getInitialSessionState } from '@andji/common/types/session-state'
import {
  getAllAgents,
  resolveNameToId,
} from '@andji/common/util/agent-name-resolver'
import { buildArray } from '@andji/common/util/array'
import { generateCompactId, pluralize } from '@andji/common/util/string'
import { closeXml } from '@andji/common/util/xml'
import { APIRealtimeClient } from '@andji/common/websockets/websocket-client'
import {
  blue,
  blueBright,
  bold,
...
import type { ApiKeyType } from '@andji/common/api-keys/constants'
import type { CostMode } from '@andji/common/old-constants'
import type { ToolResultPart } from '@andji/common/types/messages/content-part'
...
async function sendActionAndHandleError(
  ...
  try {
    ...
    console.log()
    console.log('andji is exiting due to an error.')
    console.log('Make sure you are on the latest version of andji!')
    console.log('-----------------------------------')
    console.log('Please run: npm install -g andji')
    console.log('-----------------------------------')
...
export class Client {
  ...
  async handleAddApiKey(keyType: ApiKeyType, apiKey: string): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_andji_APP_URL}/api/api-keys`,
        {
          method: 'POST',
          headers: {
    ...
    if (this.user) {
      try {
        const redeemReferralResp = await fetch(
          `${process.env.NEXT_PUBLIC_andji_APP_URL}/api/referrals`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
        ...
        if (redeemReferralResp.ok) {
          console.log(
            [
              ...
              `(pssst: you can also refer new users and earn ${CREDITS_REFERRAL_BONUS} credits for each referral at: ${process.env.NEXT_PUBLIC_andji_APP_URL}/referrals)`,
            ].join('\n'),
          )
          this.getUsage()
        } else {
      ...
      const pollInterval = setInterval(async () => {
        ...
        try {
          ...
          if (user) {
            ...
            const referralLink = `${process.env.NEXT_PUBLIC_andji_APP_URL}/referrals`
            const responseToUser = [
  ...
  private setupSubscriptions() {
    const onError = (
      ...
      if (action.error === 'Insufficient credits') {
        ...
        console.error(
          `Visit ${blue(bold(process.env.NEXT_PUBLIC_andji_APP_URL + '/usage'))} to add credits.`,
        )
        ...
        console.error(
          yellow(
            `Visit ${blue(bold(process.env.NEXT_PUBLIC_andji_APP_URL + '/usage'))} to update your payment settings.`,
          ),
        )
      } else {
        console.error(['', red(`Error: ${action.message}`)].join('\n'))
  ...
  private handleInitializationComplete() {
    ...
    console.log(
      cyan(
        `\n📋 What andji.json does:\n• ${bold(
          'startupProcesses',
        )}: Automatically runs development servers, databases, etc. when you start andji\n• ${bold(
          'fileChangeHooks',
      ...
      async (action) => {
        ...
        if (!parsedAction.success) {
          const message = [
            ...
            'If this issues persists, please contact support@andji.com',
          ].join('\n')
...
If you would like background processes (like this one) to run automatically whenever andji starts, creating a ${CONFIG_FILE_NAME} config file can improve your workflow.
Go to https://www.andji.com/config for more information.`) +
              '\n',
          )
        }
