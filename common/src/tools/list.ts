...
import type { ToolMessage } from '../types/messages/andji-message'
import type {
  ToolCallPart,
  ToolResultPart,
...
// Tool call from LLM
export type andjiToolCall<T extends ToolName = ToolName> = {
  [K in ToolName]: {
    toolName: K
...
export type andjiToolOutput<T extends ToolName = ToolName> = {
  [K in ToolName]: z.infer<(typeof $toolParams)[K]['outputs']>
}[T]
export type andjiToolResult<T extends ToolName = ToolName> = {
  [K in ToolName]: {
    toolName: K
    output: andjiToolOutput<K>
  } & Omit<ToolResultPart, 'type'>
}[T]

export type AndjiToolMessage<T extends ToolName = ToolName> = ToolMessage &
  {
    [K in ToolName]: {
      toolName: K
      content: {
