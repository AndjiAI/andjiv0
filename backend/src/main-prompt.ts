import { AgentTemplateTypes } from '@andji/common/types/session-state'
...
import type { ClientAction } from '@andji/common/actions'
...
export const mainPrompt = async (
  ...
  if (agentId) {
    ...
    const configBaseAgent = fileContext.andjiConfig?.baseAgent
    if (configBaseAgent) {
  ...
  if (!agentId) {
    // If --agent is not specified, use the spawnableAgents from the andji config or add all local agents
    updatedSubagents =
      fileContext.andjiConfig?.spawnableAgents ??
