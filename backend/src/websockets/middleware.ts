...
import type { ClientAction, ServerAction } from '@andji/common/actions'
...
protec.use(async (action, clientSessionId, ws, userInfo) => {
  ...
  if (balance.totalRemaining <= 0) {
    ...
    const message =
      balance.totalDebt > 0
        ? `You have a balance of negative ${pluralize(Math.abs(balance.totalDebt), 'credit')}. Please add credits to continue using andji.`
