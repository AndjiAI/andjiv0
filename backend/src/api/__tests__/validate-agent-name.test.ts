import { AGENT_PERSONAS } from '@andji/common/constants/agents'
import {
  describe,
  it,
  expect,
...
function createMockReq(query: Record<string, any>): Partial<ExpressRequest> {
  return { 
    query, 
    headers: { 'x-andji-api-key': 'test-api-key' } 
  } as any
}
...
describe('validateAgentNameHandler', () => {
  ...
  it('returns valid=true for published agent ids (publisher/name)', async () => {
    const agentId = 'andji/file-explorer'

    const spy = spyOn(agentRegistry, 'getAgentTemplate')
    spy.mockResolvedValueOnce({ id: 'andji/file-explorer@0.0.1' } as any)
    ...
    expect(res.jsonPayload.normalizedId).toBe('andji/file-explorer@0.0.1')
  })

  it('returns valid=true for versioned published agent ids (publisher/name@version)', async () => {
    const agentId = 'andji/file-explorer@0.0.1'
