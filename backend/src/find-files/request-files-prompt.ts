...
import { getAllFilePaths } from '@andji/common/project-file-tree'
...
async function getRelevantFiles(
  ...
  let andjiMessages = messagesWithSystem(messagesWithPrompt, system)

  // Converts assistant messages to user messages for finetuned model
  andjiMessages = andjiMessages
    .map((msg, i) => {
      if (msg.role === 'assistant' && i !== andjiMessages.length - 1) {
  ...
  let response = await promptFlashWithFallbacks(andjiMessages, {
    clientSessionId,
    userInputId,
