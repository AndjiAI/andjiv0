import { insertTrace } from '@andji/bigquery'
...
import { TOOLS_WHICH_WONT_FORCE_NEXT_STEP } from '@andji/common/tools/constants'
...
import type { AgentResponseTrace } from '@andji/bigquery'
import type { AndjiToolMessage } from '@andji/common/tools/list'
import type { AgentTemplate } from '@andji/common/types/agent-template'
import type {
  AssistantMessage,
  Message,
} from '@andji/common/types/messages/andji-message'
...
import type {
  ...
  AgentState,
  AgentOutput,
} from '@andji/common/types/session-state'
      ...
      messageHistory: [
        ...expireMessages(messageHistory, 'userPrompt'),
        {
          ...
          content: asSystemMessage(
            `The assistant has responded too many times in a row. The assistant's turn has automatically been ended. The number of responses can be changed in andji.json.`,
          ),
        },
      ],
    }
  ...
  if (clearReadFileToolResults) {
    ...
    for (const message of messageHistory) {
      if (
        ...
        const m = message as AndjiToolMessage<'read_files'>
