...
import { utils } from '@andji/internal'
import { eq } from 'drizzle-orm'

import { logger } from './logger'
import { extractAuthTokenFromHeader } from './auth-helpers'

import type { ServerAction } from '@andji/common/actions'
...
export const checkAdmin = async (
  ...
  if (!authToken) {
    return res.status(401).json({ error: 'Missing x-andji-api-key header' })
  }
  ...
  const adminUser = await utils.checkUserIsandjiAdmin(user.id)
  if (!adminUser) {
    logger.warn(
