...
function simplifyTerminalHelper(
  toolResult: andjiToolOutput<'run_terminal_command'>,
  numKept: number,
): { result: andjiToolOutput<'run_terminal_command'>; numKept: number } {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (m.role === 'system' || m.role === 'user' || m.role === 'assistant') {
      ...
      const terminalResultMessage = cloneDeep(
        m,
      ) as AndjiToolMessage<'run_terminal_command'>
  ...
  return buildArray(
    messages
      ...
      .map((m) => {
        try {
          return (
            m as AndjiToolMessage<'read_files'>
