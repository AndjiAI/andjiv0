import { consumeCreditsWithFallback } from '@andji/billing'
...
export const handleWebSearch = ((params: {
  ...
  toolCall: andjiToolCall<'web_search'>
...
}): { result: Promise<andjiToolOutput<'web_search'>>; state: {} } => {
  const {
  ...
  const webSearchPromise: Promise<andjiToolOutput<'web_search'>> =
    (async () => {
      try {
...
}) satisfies andjiToolHandlerFunction<'web_search'>
