...
import type { GetExpandedFileContextForTrainingBlobTrace } from '@andji/bigquery'
import type {
  andjiToolCall,
  andjiToolOutput,
...
export const handleFindFiles = ((params: {
  ...
  toolCall: andjiToolCall<'find_files'>
...
}): { result: Promise<andjiToolOutput<'find_files'>>; state: {} } => {
  const {
...
}) satisfies andjiToolHandlerFunction<'find_files'>
