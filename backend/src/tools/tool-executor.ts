import { endsAgentStepParam } from '@andji/common/tools/constants'
...
import { andjiToolDefs } from './definitions/list'
import { andjiToolHandlers } from './handlers/list'

import type { andjiToolHandlerFunction } from './handlers/handler-function-type'
...
export function parseRawToolCall<T extends ToolName = ToolName>(
...
): andjiToolCall<T> | ToolCallError {
  const toolName = rawToolCall.toolName

  if (!(toolName in andjiToolDefs)) {
    return {
      toolName,
  ...
  // Add the required andji_end_step parameter with the correct value for this tool if requested
  if (autoInsertEndStepParam) {
    processedParameters[endsAgentStepParam] =
      andjiToolDefs[validName].endsAgentStep
  }

  const paramsSchema = andjiToolDefs[validName].endsAgentStep
    ? (
        andjiToolDefs[validName]
    ...
    : andjiToolDefs[validName].parameters
...
export interface ExecuteToolCallParams<T extends string = ToolName> {
  toolName: T
  input: Record<string, unknown>
  toolCalls: (andjiToolCall | CustomToolCall)[]
...
export function executeToolCall<T extends ToolName>({
  ...
  const toolCall: andjiToolCall<T> | ToolCallError = parseRawToolCall<T>(
    {
      toolName,
  ...
  const handler = andjiToolHandlers[toolName] as any
  const { result: toolResultPromise, state: stateUpdate } = handler({
    ...
    requestClientToolCall: async (
      ...
      return clientToolResult.output as andjiToolOutput<T>
    },
    toolCall,
    getLatestState: () => state,
    state,
  }) as ReturnType<andjiToolHandlerFunction<T>>
