...
import { pluralize } from '@andji/common/util/string'
import z from 'zod/v4'

import { andjiToolDefs } from './definitions/list'

import type { ToolName } from '@andji/common/tools/constants'
import type { customToolDefinitionsSchema } from '@andji/common/util/file'
...
export const toolDescriptions = Object.fromEntries(
  Object.entries(andjiToolDefs).map(([name, config]) => [
    name,
...
) as Record<keyof typeof andjiToolDefs, string>
...
export const getShortToolInstructions = (
  ...
  const toolDescriptions = [
    ...(
      toolNames.filter(
        (name) => (name as keyof typeof andjiToolDefs) in andjiToolDefs,
      ) as (keyof typeof andjiToolDefs)[]
    ).map((name) => {
      const tool = andjiToolDefs[name]
