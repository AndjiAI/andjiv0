...
} from '@andji/common/project-file-tree'
import { createMarkdownFileBlock } from '@andji/common/util/file'
import { truncateString } from '@andji/common/util/string'
import { closeXml } from '@andji/common/util/xml'
import { schemaToJsonStr } from '@andji/common/util/zod-schema'
...
# andji Configuration (${andjiConfigFile})

## Schema

The following describes the structure of the \`./${andjiConfigFile}\` configuration file that users might have in their project root. You can use this to understand user settings if they mention them.

${schemaToJsonStr(andjiConfigSchema)}
...
First, read knowldge.md and ${andjiConfigFile} top level directory.
...
4. Use the 'write_file' tool to save this Markdown summary to a new file with a generated name starting with the prefix 'andji-export-' like 'andji-export-topic-of-conversation.md' in the project root directory.

Write file tool format:

<write_file>
<path>andji-export-file-name.md${closeXml('path')}
<content>
...
4. **Create the commit, ending with this specific footer:**
   \`\`\`
   Generated with andji 🤖
   Co-Authored-By: andji <noreply@andji.com>
   \`\`\`
