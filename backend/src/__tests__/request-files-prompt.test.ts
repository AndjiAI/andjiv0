import { finetunedVertexModels } from '@andji/common/old-constants'
import {
  clearMockedModules,
  mockModule,
...
describe('requestRelevantFiles', () => {
  ...
  beforeAll(() => {
    mockModule('@andji/backend/llm-apis/gemini-with-fallbacks', () => ({
      promptFlashWithFallbacks: bunMockFn(() =>
        Promise.resolve('file1.ts\nfile2.ts'),
      ),
    }))

    mockModule('@andji/backend/websockets/request-context', () => ({
