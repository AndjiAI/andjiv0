import * as analytics from '@andji/common/analytics'
...
import type { StepGenerator } from '@andji/common/types/agent-template'
...
describe('loopAgentSteps - runAgentStep vs runProgrammaticStep behavior', () => {
  ...
  beforeAll(() => {
    // Mock logger
    mockModule('@andji/backend/util/logger', () => ({
      logger: {
        debug: () => {},
        error: () => {},
        info: () => {},
    ...
    // Mock bigquery
    mockModule('@andji/bigquery', () => ({
      insertTrace: () => {},
    }))

    // Mock agent registry
    mockModule('@andji/backend/templates/agent-registry', () => ({
    ...
    mockModule('@andji/backend/async-agent-manager', () => ({
      asyncAgentManager: {
        getAgent: () => null,
        registerAgent: () => {},
        updateAgentState: () => {},
  ...
  it('should verify correct STEP behavior - LLM called once after STEP', async () => {
    ...
    const mockCheckLiveUserInput = require('@andji/backend/live-user-inputs')
  ...
  it('should respect async agent messages and continue appropriately', async () => {
    ...
    const mockAsyncAgentManager = require('@andji/backend/async-agent-manager')
  ...
  it('should pass shouldEndTurn: true as stepsComplete when end_turn tool is called', async () => {
    ...
    const mockedRunProgrammaticStep = await mockModule(
      '@andji/backend/run-programmatic-step',
      () => ({
